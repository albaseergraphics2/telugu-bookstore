...
export default function InvoicePage() {
  const { name, invoiceId } = useParams();
  ...
  const handlePDF = async () => {
    ...
    pdf.save(
      `Admin-invoice-${order.invoiceId || order._id.slice(-6).toUpperCase()
      }.pdf`
    );
  };
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        const url = `/api/admin/online/customers/${encodeURIComponent(
          name
        )}/orders/${invoiceId}`;
...
<Link
  href={`/admin/online/customers/${encodeURIComponent(name)}/orders`}
  className="back-home"
>
  ← Back
</Link>
