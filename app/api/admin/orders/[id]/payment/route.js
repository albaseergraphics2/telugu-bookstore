...
export async function POST(req, { params }) {
    try {
        ...
        if (order.orderSource !== "online") {
            return NextResponse.json({
                success: false,
                message: "Only online orders can receive payments.",
            }, { status: 400 });
        }

        const totalAmount = Number(order.totalAmount || 0) + Number(order.deliveryCharge || 0);
        const currentPaidAmount = Number(order.paidAmount || 0);
        const currentDueAmount = Number(order.dueAmount ?? totalAmount - currentPaidAmount);
        ...
        const newDueAmount = Number(Math.max(0, totalAmount - newPaidAmount).toFixed(2));
        ...
        order.paymentHistory.push({
            amount,
            paymentMethod,
            utrNumber: paymentMethod === "UPI" || paymentMethod === "Bank Transfer" ? referenceNumber : "",
            paidAt: new Date(),
        });
